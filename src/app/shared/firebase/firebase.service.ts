...
export class FirebaseService {
  ...
  isExistingUser$(username: string): Observable<boolean> {
    ...
    return fromPromise(get(query(usersRef, orderByChild('username'), equalTo(username)))).pipe(
      map((userSnapshot) => userSnapshot.exists()),
    );
  }
  ...
  getUser$(username: string): Observable<User | null> {
    ...
    return fromPromise(get(query(usersRef, orderByChild('username'), equalTo(username)))).pipe(
      map((userSnapshot) => {
        ...
        return queryResult ? (Object.values(queryResult)[0] as User) : null;
      }),
    );
  }
  ...
  createUser$(username: string, password: string): Observable<DatabaseReference> {
    const usersRef = ref(this.database, 'users');
    return from(push(usersRef, { username, password })).pipe(
