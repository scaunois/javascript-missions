...
export class AuthenticationService {
	...
	isAuthenticated = toSignal(
		this.authenticatedUserSubject$.pipe(map(user => user !== null)),
		{requireSync: true},
	);

	authenticatedUser = toSignal(this.authenticatedUserSubject$,
		{requireSync: true});

	logIn$(username: string, password: string): Observable<boolean> {
		return this.firebaseService.getUser$(username)
			.pipe(
